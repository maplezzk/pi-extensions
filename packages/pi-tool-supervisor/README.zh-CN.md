...
| `## 归属与 severity…` 段落里的编号 | **不参与判断**——那是「怎么报告」，不是「什么代码算违规」 |
| 条款开头的 `[error]` / `[warning]` | 会从判据里剔掉；本后端不分级，**命中即阻断** |
| 条款里的 `**粗体**` | 当规则名，拼成 `{段落} {编号} {标题}`，就是 finding 里的 `ruleGroup` |
| front matter 的 `threshold` | 该文件全部条款的命中阈值；不写默认 `0.85` |
| 一个 reviewer 挂多个规则文件 | 判断 id 加文件序号前缀（`f1_rule_1`）；跨文件重号是常态，不告警，同一个文件里编号重复才告警 |
| 其他散文、`## 输出` 这类段落 | 不影响判断 |

`## 归属与 severity` 这个段落名在常见规则文件里是固定写法，段落内的编号一律跳过，所以「
...
- **不分 severity。** 同一份规则文件里 `[warning]` 条款在 `model` 后端只提示，在 `typesafe` 后端会阻断。
