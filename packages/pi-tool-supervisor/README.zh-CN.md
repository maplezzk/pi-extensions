...
它会打开配置面板：顶层是总开关、超时、文件上下文与规则行数上限、TypeSafe 连接，然后是每个 reviewer 一行。选中某个 reviewer 打开它的字段页（启用、名称、审查引擎、审查模型、规则文件、工具范围、触发阶段、条件模块、文件匹配）以及删除动作。审查模型那一行回车打开本会话可用模型的可搜索列表。每改一项立即写盘，无需 `/reload`。
