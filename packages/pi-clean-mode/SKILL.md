...
| 运行中的提示块把左侧轨道切断 | 提示条目应该带上 `│ ` 前缀（`shouldRailNoticeEntry`：总开关开 + 提示条目 + 非收起态 + 运行中）。若没带上：看提示条目的 `customType` 是否真是 `NOTICE_ENTRY_TYPE`（别的扩展自己写的 custom entry 不走这条路），或它首次渲染时是否已经不在运行中（归属只看首次渲染那一帧，之后不补） |
...
| 运行期间的提示块 | 前缀 `│ `（`renderGutterPrefix`，弱化色）+ 提示块本体（按 `width - 2` 渲染） |
