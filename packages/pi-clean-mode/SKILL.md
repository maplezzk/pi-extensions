...
| 组头永远是「探索 · N 步」 | 主词按组内**过半分类**选（`getGroupActivityLabel` → `dominantActivityClass`）；没有哪一类过半时改用构成文案（`activityCompositionLabel`，`运行命令 2 · 读取文件 1`）；只有连计数都拿不到（没传 `activity`）时才退回通用词，那是预期行为不是 bug；分类由 `registerActionToolCall` 的 `activity` 字段按组累加 |
