...
| 分类计数又单独占一行 | 计数由 `activity-area.ts` 接在活动块最后一个非空行尾（`activityCountersNote` + `appendActivityCountersNote`），不另开一行也不进组头；接在**已渲染**的行上，所以单条组去掉动作行之后不会跟着消失，也不占行数预算 |
...
| 动作组头 | 细竖条 `│`（弱化色）+ 普通字重文案 + 紧跟其后的强调色箭头 |
| 成员命令行 | 树形前缀（`├─`，分支符用 `dim` 色）+ 动作摘要 + 紧跟其后的强调色箭头；不画竖条（它长在组头的竖条下面） |
| 正文 / 工具行 | 不加任何装饰；可见的工具行也会在首行尾部加同款箭头（`insertToolRowArrow`） |

**层级靠左侧竖条 + 字重，不靠底色。** 三条竖条落在同一列，连起来是一条从上到下的轨道。`RUN_GUTTER` / `GROUP_GUTTER` / `GUTTER_GAP` 在 `header-style.ts` 里只定一次，`activity.ts` 的 `TREE_INDENT` 为空串就是为了让 `├─` 与两级竖条同列 —— 改竖条只需改一处，但**必须同时确认树形前缀仍与它同列**。
