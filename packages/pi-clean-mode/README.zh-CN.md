...
| `showActivityArea` | 在对话流末尾显示实时活动区。 |
...
它长在 transcript 末尾，跟着对话一起滚，而不是钉在编辑器上方的状态条 —— 向上翻历史时它跟着内容走开，和 Codex 客户端的做法一致。

内容全部来自真实事件 —— 正在跑的工具、它的最新输出行、思考头部、以及分类计数。并行调用会收成一行汇总。

两条实现约束：

1. **内容不变就完全不重绘。** 每 tick 先把行渲染成字符串与上一 tick 比较，一致时**根本不请求重绘**；重绘代价落在内容高度变化上。
...
行是把 transcript 容器的 `render` 结果尾部接上去的。容器靠遍历 TUI 组件树定位（唯一直接持有 assistant 消息的那一个），判定用属性特征而不是 `instanceof`，避免扩展与 Pi 各自加载一份 `pi-tui` 时认不出来。
