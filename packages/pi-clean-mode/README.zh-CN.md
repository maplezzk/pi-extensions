...
实现上分两处渲染，但共用同一份行数据（`runtime.activityArea.lines`）：当前组的工具行渲染时把活动块接在自己末尾（`component-patches.ts` 的 `appendActivityTail`，只有最后一条可见行会接），轮首折叠头子组件只输出运行级时间（同文件的 `createRunHeaderComponent` 读 `getRunStatusLines`），组头 chip 的分类计数则由 `getActivityCounters`（组装自 `formatActivityCountersSuffix`）拼上去。三处都只认「当前组」与「当前轮的轮首承载者」，所以历史轮次不会重复显示同一块内容。只有轮首那条运行级横条铺底色（`bandActivityHead`），活动块里的行原样输出，补位空行也保持空行。
