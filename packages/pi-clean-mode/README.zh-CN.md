...
用户：帮我改一下 xxx
 ...
 运行命令 npm test                    ← 最新一条动作
   ⠋ 处理中 · 42s · 读取 4 · 搜索 3     ← 活动块：接在最新那条动作下面
     ◐ 思考  正在追踪 token 失效路径…
     ⠋ 运行命令 npm test
       ↳ 12 passing
```

- **顶部（运行级横条）**：只报整轮时间，不带计数、不带思考与工具细节。运行中是 `⠋ 处理中 · 42s`，结束后换成 `用时 42s · 3 步 ▶` —— 同列同款，状态切换只换文案。
...
活动块的第一行是状态横条（`处理中` / `并行执行` + 耗时 + 分类计数），计数里为 0 的项不显示；后面依次是思考头部、正在执行的工具（并行时逐条列出）与其最新输出行。内容全部来自真实事件，不生成推测出来的进度。
...
实现上分两处渲染，但共用同一份行数据（`runtime.activityArea.lines`）：当前组的工具行渲染时把活动块接在自己末尾（`component-patches.ts` 的 `appendActivityTail`，只有最后一条可见行会接），轮首折叠头子组件只输出运行级时间（同文件的 `createRunHeaderComponent` 读 `getRunStatusLines`）。两边都只认「当前组」与「当前轮的轮首承载者」，所以历史轮次不会重复显示同一块内容。活动块首行的底色用 `styler.band` 铺上（只铺首行，补位空行保持空行）。
