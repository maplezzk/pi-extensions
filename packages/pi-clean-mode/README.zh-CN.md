...
```
用户：帮我改一下 xxx
  ⠋ 处理中 · 42s                     ← 顶部：整轮时间 + 状态（只此一处写「处理中」）
 探索 · 5 步 ▶                        ← 当前组头（收起时组头是该组唯一可见行）
 运行命令 ls -la ▶
 运行命令 npm test                    ← 最新一条动作
   ⠋ 读取 4 · 搜索 3                   ← 活动块：接在最新那条动作下面，只报计数
     ◐ 思考  正在追踪 token 失效路径…
     ⠋ 运行命令 npm test
       ↳ 12 passing
```
...
**「处理中」与耗时只在顶部出现一次**（顶部横条需要它们回答「整轮跑了多久」）。活动块的第一行只报分类计数，计数里为 0 的项不显示；一项计数都没有时这一行不画，块直接从思考行或动作行开始（不会铺出一条空底色块）。计数行后面依次是思考头部、正在执行的工具（并行时逐条列出）与其最新输出行。内容全部来自真实事件，不生成推测出来的进度。
