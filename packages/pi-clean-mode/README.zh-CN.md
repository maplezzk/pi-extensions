...
| 层级 | 视觉 | 含义 |
|---|---|---|
| 运行级折叠头 | 整行铺满底色的**横条** + 靠右箭头与快捷键提示 | 这里收了一整轮 |
| 动作组头 | 与折叠头**文案同列** + 只包住文字的**底色标签** + 靠右箭头 | 这里收了一次或一组工具调用 |
| 正文与工具行 | 不铺底色，保持 Pi 原本的样子 | 没被收起来的内容 |

单条动作的标签直接用这条动作自己的摘要（「运行命令 ls -la」），多条才汇总成「探索 · N 步」。箭头 `▸` / `▾` 贴在行的右边缘，同时表示当前状态和「可以点」。
