...
agent 运行期间会显示一小块「现在在做什么」。它跟着**当前 turn 的动作组头**走，所以进度总是贴在最新动作旁边：

```
用户：帮我改一下 xxx
  ⠋ 处理中 · 42s · 读取 4 · 搜索 3 · 命令 1   ← 首行是与「用时」同款的底色横条
    ◐ 思考  正在追踪 token 失效路径…
    ⠋ 运行命令 npm test
      ↳ 12 passing
 探索 · 5 步 ▼
...
- 本轮还没有工具调用（屏幕上还没有组头可挂）；
- 运行级收起（工具行整行隐藏）。

其余时候它一直挂在当前组头上 —— 新 turn 开新组，活动块就跟着新组头往下走，不用回头往上看。运行结束后活动块清空，那个位置换成 `用时` 横条：两者的首行同列、同为底色横条，所以状态切换时变的是文字，不是版式。

活动块的第一行永远是状态横条（`处理中` / `并行执行` + 耗时 + 分类计数），计数里为 0 的项不显示；后面依次是思考头部、正在执行的工具（并行时逐条列出）与其最新输出行。内容全部来自真实事件，不生成推测出来的进度。
...
实现上分两处渲染，但共用同一份行数据（`runtime.activityArea.lines`）：当前组的工具行渲染时把活动行插在组头行上方（`component-patches.ts` 的 `buildGroupHeadLines`），轮首折叠头子组件只在上面那两个回落条件下输出它（同文件的 `createRunHeaderComponent`）。两边都只认「当前组」与「当前轮的轮首承载者」，所以历史轮次不会重复显示同一块活动行。首行的底色用 `styler.band` 铺上（只铺首行，补位空行保持空行）。
