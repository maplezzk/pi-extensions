...
它和运行级「用时」横条共用同一个槽位：运行中耗时还不知道，这个位置放活动块；运行结束后活动块清空，同一个位置换成 `用时` 横条。两块的首行同列、同为底色横条，所以状态切换时变的是文字，不是版式。
...
行是轮首折叠头子组件输出的（见 `component-patches.ts` 的 `createRunHeaderComponent`）：同一个组件里运行中画活动块、运行结束画 `用时` 横条，两者不会同时出现（耗时在 `agent_settled` 里才写入，写完活动行立刻被清空）。只有当前那一轮的承载者会输出活动行，否则历史轮次会把同一块活动行重复显示一遍。首行的底色由同一个组件用 `styler.band` 铺上（只铺首行，补位空行保持空行）。
