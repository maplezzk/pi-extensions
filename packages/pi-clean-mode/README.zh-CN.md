...
- **组头 chip**：`探索 · N 步` 后面直接接上本轮的分类计数（`· 读取 3 · 命令 2`）。它不另占一行 —— 单独一行时就变成“步数”和“分类计数”两处在数同一件事，加上顶部横条就是三处报进度。只有汇总成「N 步」的组头才带它，且只给当前组带（计数是本轮累计值）。
