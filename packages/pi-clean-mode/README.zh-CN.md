...
用户：帮我改一下 xxx
 ...
 运行命令 ls -la ▶
 运行命令 npm test                ← 最新一条动作
     ◐ 思考  正在追踪 token 失效路径…   ← 活动块：接在最新那条动作下面
     ⠋ 运行命令 npm test
       ↳ 12 passing
```
...
活动块里全是普通行（不铺底色、不重复顶部的时间与分类计数）：思考头部、正在执行的工具（并行时逐条列出）与其最新输出行。内容全部来自真实事件，不生成推测出来的进度。
...
实现上分两处渲染，但共用同一份行数据（`runtime.activityArea.lines`，并标明哪几行在报「正在跑什么」）：当前组的工具行渲染时把活动块接在自己末尾（`component-patches.ts` 的 `appendActivityTail`，只有最后一条可见行会接），轮首折叠头子组件只输出运行级时间（同文件的 `createRunHeaderComponent` 读 `getRunStatusLines`），组头 chip 的分类计数则由 `getActivityCounters`（组装自 `formatActivityCountersSuffix`）拼上去。组内只有一条时渲染去掉动作名的版本（只留思考与输出尾巴），多条时才逐条列出正在跑什么。三处都只认「当前组」与「当前轮的轮首承载者」，所以历史轮次不会重复显示同一块内容。只有轮首那条运行级横条铺底色（`bandActivityHead`），活动块里的行原样输出，补位空行也保持空行。
