...
import { buildCanceledStatusLine, buildNotCompletedStatusLine, buildStatusLine, colorizeText } from "../src/status-line.ts";
...
test("用户打断时明确写成「未判定」，与「判定为可停止」区分开", () => {
  const canceled = buildCanceledStatusLine();
  assert.equal(canceled.color, "dim");
  assert.match(canceled.text, /已打断|interrupted/);
  assert.match(canceled.text, /未判定|not judged/);

  const skipped = buildStatusLine({ kind: "skipped", code: "canceled", used: 0, limit: 2, budget: "0/2" });
  assert.equal(skipped.text, canceled.text);
});

test("非正常结束（失败或残缺）用另一条文案，不冒充「已打断」", () => {
  const notCompleted = buildNotCompletedStatusLine();
