...
- **后台任务在跑时判为正常等待**：如果 agent 本轮启动了后台任务（`subagent`、`subagent_resume`、`workflow` 这类调用后立刻返回、结果稍后自动送回会话的工具），并且没有证据表明结果已经处理完，判定提示词要求判「可以停止」。后台任务跑完会自己唤醒会话，此时催促只会多打一轮；这一条优先于其它判定条目。
