...
| 结论行 | 颜色 | 含义 |
| --- | --- | --- |
| `⚖️ 判定可停止 · 置信度 92%`（下一行 `理由：…`） | 绿 | 判定为正常结束，没有干预。理由默认显示在结论下面一行，不用展开。 |
| `⚖️ 判定该继续 · 已催促 1/2` | 黄 | 判定为提前停止，已自动发催促（1/2 = 已催 1 次，上限 2 次）。 |
| `⚖️ 催促次数用尽（2/2）· 不再干预` | 灰 | 本轮干预次数用尽，不再干预。 |
| `⚖️ 已打断 · 未判定` | 灰 | 你按 Esc 打断了这一轮，判定主动让路。 |
| `⚖️ 本轮未正常结束 · 未判定` | 灰 | 这一轮以失败或残缺结束（不是 agent 自己停下）。 |
| `⚖️ 判定失败` | 红 | 判定调用失败。 |

判「可停止」时，结论下面默认再跟一行理由（这条最需要排查，不用展开就能看到）；其余细节——已发送的催促文本、失败原因、结束原因——收在展开里：**按 `Ctrl+O`** 展开工具输出时会一起展开，平时不占地方。
