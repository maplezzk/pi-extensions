...
- 每个有判定的轮次只发**一条**提示块：正文一行（如「⚖️ 判定可停止 · 置信度 92%」）。判「可停止」时默认在下面多跟一行 `理由：…`（不展开也能排查）；其余细节（已发送的催促、失败原因、结束原因）仍在 `Ctrl+O` 展开里。调这个时不要退回多条提示，也不要把 stop 理由塞回展开里。
  Each judged turn emits exactly **one** notice block: a one-line body (`⚖️ stop accepted · confidence 92%`). For an accepted stop the block also shows a `Reason: …` line by default (auditable without expanding); the sent continuation, the failure, and the stop reason stay in the `Ctrl+O` details. Do not go back to multiple notices per turn, and do not move the stop reason back into the details.
