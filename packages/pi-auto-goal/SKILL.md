...
- 每个有判定的轮次只发**一条**提示块：正文一行（如「⚖️ 判定可停止 · 置信度 92%」），判定理由/已发送的催促/失败原因/结束原因都在 `Ctrl+O` 展开的细节里，平时不占地方。调这个时不要退回多条提示。
  Each judged turn emits exactly **one** notice block: a one-line body (`⚖️ stop accepted · confidence 92%`) with the reason, the sent continuation, the failure, or the stop reason in the `Ctrl+O` details. Do not go back to multiple notices per turn.
...
- 判定提示词把「启动后台任务后等结果」当作可以停止：本轮调用过 `subagent`、`subagent_resume`、`workflow` 这类立即返回、结果稍后自动送回会话的工具，且没有证据表明结果已经处理完时，必须判 `stop`；该条优先于其它条目，有后台任务在跑时不要再催。
  The judge prompt accepts waiting on background work: when this round called tools like `subagent`, `subagent_resume`, or `workflow` (they return immediately and deliver their result automatically later) and nothing shows that result was handled, the verdict must be `stop`; this rule outranks the others, so never continue while background work is in flight.
