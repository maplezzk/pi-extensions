...
- 每个有判定的轮次只发**一条**提示块：正文一行（如「⚖️ 判定可停止 · 置信度 92% · 理由：…」）。判「可停止」的理由直接写在正文里（不展开也能排查）；已发送的催促、失败原因、结束原因仍在 `Ctrl+O` 展开的细节里。调这个时不要退回多条提示，也不要把 stop 理由塞回细节。
  Each judged turn emits exactly **one** notice block: a one-line body (`⚖️ stop accepted · confidence 92% · reason: …`). The reason for an accepted stop lives in the body so it is auditable without expanding; the sent continuation, the failure, and the stop reason stay in the `Ctrl+O` details. Do not go back to multiple notices per turn, and do not move the stop reason back into the details.
