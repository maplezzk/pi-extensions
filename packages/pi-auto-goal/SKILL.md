...
- 命中提前停止时，催促是**system 提示**，不是用户消息：先发一条 `display: false` 的自定义消息（`customType: "auto-goal-nudge"`）触发新一轮，再由 `context` 钩子把它换成同位置的 system 消息；催促轮结束后该消息不再进入上下文。旧运行时（消息列表里没有 system 消息，即 Pi 0.85 及更早）退回自定义消息本身，不静默丢指令。改投递方式时保留这两条：会话区不能出现伪造的用户消息，催促不能残留到后续轮次的系统提示里。
  A premature-stop nudge is a **system directive**, not a user message: an invisible custom message (`customType: "auto-goal-nudge"`, `display: false`) triggers the turn, and the `context` hook swaps it for a system message in the same position; once that turn settles it no longer enters context. On older runtimes (no system message in the list, i.e. Pi 0.85 and earlier) it falls back to the custom message itself rather than dropping the instruction. Keep both properties when changing delivery: no fake user message in the transcript, and no stale nudge left in later system prompts.
