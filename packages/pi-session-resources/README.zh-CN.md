...
- Pi fullscreen 模式下在输入框上方显示一行 `查看资源` 按钮；点击打开资源面板，之后可以完全用鼠标切换 Tab、查看高亮和打开资源。
...
│   tests/index.test.ts                               修改 │
├────────────────────────────────────────────────────────┤
│ ←/→ 类型 · ↑/↓ 选择 · 点击打开 · Shift+点击插入          │
╰────────────────────────────────────────────────────────╯
```

- 点击按钮即可打开面板，不需要先键入 `#`。这种打开方式没有查询词，因此再输入字符会回到输入框，而不是继续筛选。
- Enter 仍然可以插入当前引用，键盘和鼠标可以混用。
- 点击 Tab 切换资源类型；鼠标悬停时按钮、Tab 和资源行会高亮。
- 点击资源行会打开资源，效果与 Cmd+click 点击 OSC 8 链接一致：Pi 的 fullscreen 渲染器会对被点中的链接调用自带的打开器。
- Shift+点击（或 Ctrl+点击）资源行则改为插入引用，不打开目标。
