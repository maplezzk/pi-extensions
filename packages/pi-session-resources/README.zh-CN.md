...
面向 [Pi 编码助手](https://github.com/earendil-works/pi) 的被动会话资源引用扩展。它从成功的工具活动中收集文件、网页和 PR/MR；在输入框键入 `#` 时，通过 Pi 内置自动补全 UI 筛选并插入资源引用。
...
- 复用 Pi 内置自动补全列表，按资源标签、目标、类型、动作和来源工具模糊筛选最近资源。
...
```text
请检查 #ind
        → ▤ src/index.ts             FILE · 读取
          ▤ tests/index.test.ts      FILE · 修改
          ⎇ owner/repo#93            PR/MR · 创建
```

- 继续在 `#` 后输入可实时筛选候选。
- Tab 切换到下一个候选，Shift+Tab 切换到上一个候选；到达列表边界后循环。
- ↑/↓ 同样可以选择候选。
- Enter 插入当前引用，Esc 关闭列表。
- 每次最多显示 12 个最近匹配资源。

文件引用使用会话中的显示路径，例如 `#src/index.ts`；含空格的路径会插入为 `#"docs/design notes.md"`。网页和 PR/MR 会插入完整 URL。引用只是普通提示文本，不会额外读取文件或向会话写入隐藏上下文。

候选标签本身是 OSC 8 链接。Pi fullscreen 模式可直接鼠标点击打开；普通模式下许多终端需要 Cmd/Ctrl-click。不支持 OSC 8 的终端仍会显示可读标签。Pi 当前未向扩展自动补全项公开鼠标命中选择 API，因此鼠标点击用于直接打开目标，候选插入仍使用键盘确认。

## 命令

```text
/config:session-resources             显示 # 引用使用提示
/config:session-resources enable      启用 # 资源补全
/config:session-resources disable     禁用 # 资源补全
```
