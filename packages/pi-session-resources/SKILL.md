...
先成功调用一次文件工具或产生结构化 URL 的工具，再输入 `#` 观察候选并用 Enter 插入。引用只是普通提示文本，不会自动读取文件。fullscreen 鼠标路径需要真实终端驱动：确认按钮出现、点击后展开面板、点击 Tab 切换类型、点击资源行由 Pi 自身打开链接、Shift+点击插入引用。非 TUI、非 fullscreen 或未实际触发时报告 `NOT_RUN`，不能用命令提示代替选择器证据。
