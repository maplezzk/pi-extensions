...
The configuration file is read when the extension loads. Use `/config:notifications` to open the TUI settings panel, or run `/config:notifications reset` to restore defaults. Changes made in the panel are saved and applied immediately; no `/reload` is needed. A missing file uses the default `terminal-notifier` configuration. A malformed file is reported in Pi and uses the default configuration.
