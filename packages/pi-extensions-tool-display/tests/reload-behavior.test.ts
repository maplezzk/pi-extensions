...
function simulateReload(capturedHandlers: CapturedHandler[]): void {
  const shutdown = capturedHandlers.find((handler) => handler.event === "session_shutdown");
...
test("1: after reload, new lifecycle handlers are registered", () => {
  ...
  // Simulate Pi's reload lifecycle before loading the extension again.
  simulateReload(capturedHandlers);
  toolDisplayExtension(api);
