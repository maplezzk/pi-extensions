...
- Provides `session_squash`: accepts the summary and continuation mode, then squashes the conversation from a chosen message to free up context. Nothing is deleted; use `/tree` to go back. The resulting snapshot message is collapsed to one line by default (`[session] ▶ Squash snapshot · from #N · X tokens before`); press `Ctrl+O` or click the line to expand the full snapshot (the arrow turns into `▼`), so the last answer before the squash is not pushed off screen. The leading `[session]` is the same source prefix other extensions use for their notice blocks, and the expand direction is an arrow rather than a `Ctrl+O` label because renderers cannot tell fullscreen from regular mode. The expanded view shows only the task-state body, not the continuation instruction meant for the receiving agent.
