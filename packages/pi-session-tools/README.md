...
- Provides `session_squash`: accepts the summary and continuation mode, then squashes the conversation from a chosen message to free up context. Nothing is deleted; use `/tree` to go back. The resulting snapshot message is collapsed to one line by default (`▸ Squash snapshot · from #N · X tokens before`); press `Ctrl+O` or click the line to expand the full snapshot, so the last answer before the squash is not pushed off screen. The expanded view shows only the task-state body, not the continuation instruction meant for the receiving agent.
