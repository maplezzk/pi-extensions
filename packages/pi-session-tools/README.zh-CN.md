...
也可用交互式命令 `/config:session-tools`（别名 `/pi-session-tools`）配置。不带参数时打开配置面板，覆盖两个字段：「强制压缩比例」回车打开候选列表（含「关闭强制」与常见比例），「提醒阈值」回车打开预填当前值的输入框。改动立即写盘并应用到当前会话，无需 `/reload`。
...
也可执行 `/config:session-tools force 0.9`；使用 `/config:session-tools force off` 关闭。强制压缩不再接受 `"90%"` 这类百分比字符串。在配置面板里选「关闭强制」同样会立即退出强制模式并恢复原工具集合。
