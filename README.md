...
| [`pi-hud`](./packages/pi-hud) | Shows a live session elapsed timer in the working spinner plus per-turn and total run summaries. | [English](./packages/pi-hud/README.md) · [中文](./packages/pi-hud/README.zh-CN.md) |
| [`pi-model-discovery`](./packages/pi-model-discovery) | Discovers models from `{baseUrl}/models` for providers marked with `discoverModels` in models.json, with a persistent startup cache and a manual refresh command. | [English](./packages/pi-model-discovery/README.md) · [中文](./packages/pi-model-discovery/README.zh-CN.md) |
